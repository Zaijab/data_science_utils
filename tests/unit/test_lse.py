...
def test_log_sum_exp_equivalence() -> None:
    ...
    def exp(logposterior_weights):
        """
        Computes: w -> exp(w) / sum(exp(w)) in a numerically stable way per measurement
        """
        m = jnp.max(logposterior_weights, axis=1, keepdims=True)
        g = m + jnp.log(
            jnp.sum(jnp.exp(logposterior_weights - m), axis=1, keepdims=True)
        )
        posterior_weights = jnp.exp(logposterior_weights - g)
        posterior_weights = posterior_weights / jnp.sum(
            posterior_weights, axis=1, keepdims=True
        )
        return posterior_weights

    lse_original = exp(weights)
    lse_unstable = jnp.exp(weights) / jnp.sum(jnp.exp(weights), axis=1, keepdims=True)
    lse_scipy = jnp.exp(
        weights - jax.scipy.special.logsumexp(weights, axis=1, keepdims=True)
    )
