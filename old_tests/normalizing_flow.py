...
class SimpleNet(nnx.Module):
    def __init__(self, output_dim, rngs, name=None):
        ...
        self.layer1 = nnx.Linear(
            2, 32, rngs=rngs, dtype=jnp.float64, param_dtype=jnp.float64
        )
        self.layer2 = nnx.Linear(
            32, output_dim, rngs=rngs, dtype=jnp.float64, param_dtype=jnp.float64
        )
...
print(
    f"Sum of log determinants: {jnp.mean(ldj_forward + ldj_backward):.8f} (should be close to 0)"
)
