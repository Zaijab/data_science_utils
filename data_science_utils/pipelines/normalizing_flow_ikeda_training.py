...
rngs = nnx.Rngs(0)
dense_test = DenseNetwork(
    input_dim=2, hidden_dim=10, num_hidden_layers=3, output_dim=10, rngs=rngs
)
...
coupling_test = CouplingLayer(
    input_dim=10, hidden_dim=4, num_hidden_layers=2, swap=False, rngs=rngs
)
    ...
    def __call__(self, x, reverse=False):
        ...
        if not reverse:
            ...
            def solve_single(y_single):
                ...
                x_single = jax.scipy.linalg.solve_triangular(U, z, lower=False)
...
def train_step(model, optimizer, batch):
    def loss_fn(model):
        ...
        base_log_prob = jax.vmap(logpdf_epanechnikov, in_axes=(0, None, None))(
...
model = InvertibleNN(
    input_dim=2, num_coupling_layers=2, hidden_dim=10, num_hidden_layers=2, rngs=rngs
)
...
for i in pbar:
    ...
    if i % 10**4 == 0:
        ...
        generated_data = model(
            sample_standard_epanechnikov(key, num_samples=batch.shape[0]), reverse=False
        )[0]
